...
const RequestOrderEdit = ({
  ...
  const [patientSearchQuery, setPatientSearchQuery] = useState<string>(
    orderData?.patientDetails?.patientName || ""
  );
  ...
  function normalizeRequestType(type?: string): "REGULAR" | "EMERGENCY" {
    if (type === "Pay Later") return "EMERGENCY";
    if (type === "Walk In") return "REGULAR";
    if (type === "EMERGENCY" || type === "REGULAR") return type;
    return "REGULAR";
  }
  ...
  const {
    ...
    mode: "onChange",
    defaultValues: {
      patientName: orderData?.patientDetails?.patientName || "",
      roomNumber: orderData?.patientDetails?.roomNumber || undefined,
      contactNumber: orderData?.patientDetails?.contactNumber,
      notes: orderData?.notes,
      products: [],
      type: normalizeRequestType(orderData?.type),
    },
  });

  // Initialize cart items from order data
  useEffect(() => {
    if (orderData?.itemDetails && products.length > 0) {
      const initialCartItems: CartItem[] = orderData.itemDetails
        .map((item: OrderItem, index: number) => {
          ...
          const totalUnits = item.quantityOrdered;
  ...
  }, [orderData?.itemDetails, products]);
  ...
  useEffect(() => {
    ...
    const newStatus = type === "EMERGENCY" ? "for_payment" : "pending";
  ...
  const onSubmit = async (data: TEditRequestOrderSchema) => {
    ...
    if (!data.products || data.products.length === 0) {
      toast.error("Products data is missing");
      console.error("❌ Products array is empty in form data");
      return;
    }
      ...
      <div className="bg-white w-full max-w-[600px] min-h-[95vh] max-h-[95vh] rounded-md relative overflow-hidden">
        ...
        <form className="pb-[70px] mt-6" onSubmit={handleSubmit(onSubmit)}>
          <div className="overflow-y-auto max-h-[calc(95vh-150px)]">
            ...
            >
              <div className="px-8 flex flex-col gap-4 pb-6">
                <p className="font-medium mb-3 flex items-center gap-2 text-lg">
                  Patient Information
                </p>
                <div className="search-container">
                  ...
                  {showSuggestions && searchSuggestions.length > 0 && (
                    <ul className="absolute z-10 w-full max-w-full bg-white border rounded-md shadow-md mt-1">
                ...
                <div className="flex flex-col w-full opacity-50 cursor-not-allowed">
                  <label className="text-start text-sm font-medium mb-[3px] text-gray-700">
                    Request Type
                  </label>
                  <div className="relative flex bg-gray-100 p-1 rounded-full w-full">
                    ...
                    <button
                      ...
                      className="relative w-1/2 text-sm py-1 z-10 transition-colors duration-300 text-green-900 font-medium cursor-not-allowed"
                      type="button"
                      disabled
                    >
                      Regular
            ...
            <div className="relative">
              <div className="px-8 py-4 border-t">
                ...
                <p className="text-start font-medium mb-3">
                  Search & Add Products
                </p>
          ...
          <div className="flex gap-6 bg-white border-t-2 p-4 absolute bottom-0 left-0 w-full justify-end">
            <CancelButton setIsModalOpen={setShowRequestEditModal} />
            <button
              type="submit"
